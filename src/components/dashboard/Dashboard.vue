<template>
  <div class="grid row">
    ...
    <div class="flex xs12">
      <va-card title="Seat Plan">
        <va-button icon="fa fa-print" flat slot="actions" class="mr-0" @click="downloadWithCSS" />
...
export default {
  ...
  methods: {
    ...
    async displayStudents (id) {
      ...
      this.sortNames = students.studentsList.sort(function (a, b) {
      ...
      for (let x = 0; x < 10; x++) {
        for (let y = 0; y < 5; y++) {
          if (students.studentsList.length > y * 10 + x) {
            var name = this.sortNames[y * 10 + x]
            this.layout.push({
              x: x * 2,
              y: y * 2,
              w: 2,
